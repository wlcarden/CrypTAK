...
class MainConfig:
    ...
    # FTS 2.2.1 patch: AllowedCLIIPs was removed from the config schema but the
    # REST API still references it. Return a permissive default so /Clients
    # and other endpoints don't return 500.
    # _AllowAllIPs matches any IP via __contains__, since FTS REST API is
    # only accessible on the LAN/Docker network (not exposed to internet).
    class _AllowAllIPs:
        def __contains__(self, item):
            return True

    _MISSING_DEFAULTS = {
        'AllowedCLIIPs': _AllowAllIPs(),
    }

    def get(self, name):
        if name in self._values:
            return self._values[name]
        elif name in self._MISSING_DEFAULTS:
