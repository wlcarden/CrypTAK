...
class EventTableController(TableController):
    ...
    def create(self, session, object):
        ...
        # PATCH: changed session.add() -> session.merge() to handle reconnecting clients.
        # session.add() issues an INSERT which fails with a duplicate-key error when the
        # same client UID reconnects and the row already exists. session.merge() performs
        # an UPSERT (SELECT then INSERT or UPDATE), avoiding the IntegrityError.
        session.merge(row)
        session.commit()
        return row
